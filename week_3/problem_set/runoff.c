    ...
    while (true)
    {
        ...
        if (tie)
        {
            for (int i = 0; i < candidate_count; i++)
            {
                ...
                {
                    printf("Election ended in tie - %s won!\n", candidates[i].name);
                }
            }
            break;
        }
...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            printf("Candidate %s won with %i votes, eliminated: %d \n", candidates[i].name, candidates[i].votes, candidates[i].eliminated);
...
bool is_tie(int min)
{
    for (int i = 0; i < candidate_count; i++)
    {
        // check if candidate is not eliminated and doesn't have least votes
